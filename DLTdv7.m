...
% Version 7 - Tyson Hedrick 7/13/18
...
% 2018-07-13 - start DLTdv7 with a number of feature enhancements in mind
...
if nargin==0 % no inputs, just fix the path and run the gui
  ...
  if str2double(v(1:3))<7
    beep
    disp('DLTdv7 requires MATLAB version 7 or later.')
    return
  end
  ...
  if exist('mmreader')~=2 && exist('VideoReader')~=2
    beep
    disp('*** DLTdv7 is currently in development and has some bugs and')
    disp('limitations not present in DLTdv5, it also has many new')
    disp('features and is probably the best platform for most users.')
    disp('***')
    disp('DLTdv7 requires mmreader or VideoReader to read AVIs or other')
  ...
  if strcmpi(mfilename,'DLTdv7')==0
    beep
    disp('Error: This program must be named "DLTdv7.m" to function properly.')
    return
  end
  ...
  if inPath==0 % didn't find a match
    % add DLTdv7's directory to the end of the path
    addpath(dlttrim,p1);
...
elseif nargin==3 % switchyard call, data & subfunction specific data
  ...
  disp('DLTdv7: incorrect number of input arguments')
  return
end
...
switch call
  ...
  case {99} % Initialize the GUI
    ...
    h(1) = figure('Units','characters',... % control figure
      ...
      'Name','DLTdv7 controls','NumberTitle','off',...
      'Position',[10 10 width top],'Resize','off',...
      'HandleVisibility','callback','Tag','figure1',...
      'Userdata',[],'Visible','on','deletefcn','DLTdv7(13)',...
      'interruptible','on');
    ...
    h(6) = uicontrol('Parent',h(1),... % initialize button
      'Units','characters','Position',[2.2 top-2.73 15 1.8],'String',...
      'New project','Callback','DLTdv7(0)','ToolTipString', ...
    ...
    h(7) = uicontrol('Parent',h(1),... % Save data button
      ...
      'Export points','Tag','pushbutton3',...
      'Callback','DLTdv7(5)','Enable','off');
    ...
    h(12)=uicontrol('Parent',h(1),... % Color display checkbox
      ...
      'colorVideos','enable','off','Callback','DLTdv7(15)');
    ...
    h(14) = uicontrol('Parent',h(1),'Units','characters',... % quit button
      ...
      'Tag','pushbutton4','Callback','DLTdv7(11)','enable','on');
    ...
    h(15)=uicontrol('Parent',h(1),... % background subtraction checkbox
      ...
      'backgroundSubtraction','enable','off','Callback','DLTdv7(16)');
    
    h(16) = uicontrol('Parent',h(1),... % load project button
      'Units','characters','Position',[2.2 top-5.23 15 1.8],'String',...
      'Load project','Callback',@dvLoadProject_v1,'ToolTipString', ...
      'Load a previously saved DLTdv7 project','Tag','pushbutton1');
    ...
    h(21) = uicontrol('Parent',h(1),... % frame slider label
      'Units','characters','BackgroundColor',[0.568 0.784 1],...
      'HorizontalAlignment','left','Position',[35 top-11.73 12.6 1.1],...
      'String','/NaN','Style','text','Tag','text6');
    
    h(8) = uicontrol('Parent',h(1),... % frame text box
      'Units','characters','BackgroundColor',[1 1 1],'Position',...
      [22 top-11.73 12 1.5],'String','0','Style','edit','Tag','edit3',...
      'Callback','DLTdv7(7)','enable','off');
    
    h(22) = uicontrol('Parent',h(1),... % load points button
      'Units','characters','Position',[21 top-5.23 14.4 1.7],'String',...
      'Load points','Tag','pushbutton5','enable','off','Callback',...
      'DLTdv7(6)');
    ...
    h(32) = uicontrol(... % Current point pull-down menu
      ...
      [20.6 top-17.33 12 1.6],'String',' 1','Style','popupmenu',...
      'Value',1,'Tag','popupmenu1','Enable','off','Callback',...
      @fullRedrawUI);
    
    h(19) = uicontrol('Parent',h(1),... % add-a-point button
      'Units','characters','Position',[34.2 top-17.33 16 1.6],'String',...
      'Add a point','Callback','DLTdv7(12)','ToolTipString', ...
    ...
    h(55)=uicontrol('Parent',h(1),... % Centroid finding checkbox
      ...
      'findCentroidBox','enable','off','Callback','DLTdv7(14)');
    ...
    h(59) = uicontrol(... % Centroid color menu
      'Parent',h(1),'Units','characters','Position',...
      [41 top-38.43 12 1.6],'String',{'black','white'},'Style',...
  ...
  case {0} % Initialize button press from the GUI
    ...
    % create movie figures and axes - see the subfunction for all graphics
    % positioning, handles, etc.
    uda = dvMakeMovieWindows(uda);
    h = uda.handles;
    ...
    for i=1:uda.nvid
      ...
      cdataCacheIdx{i}=nan(cacheDepth,1);
    end
    ...
    for i=1:uda.nvid
      ...
      % set framerate
      set(h(450+i),'String',num2str(movieInfo.frameRate));
      ...
      uda.movFrameRates(1,i)=movieInfo.frameRate;
      ...
      set(get(h(i+300),'Children'),'ButtonDownFcn','DLTdv7(3)');
    ...
    uda.movFrameRates(1,i)=movieInfo.frameRate;
    uda.movFrameRateRatios=uda.movFrameRates./uda.movFrameRates(1);
    ...
    if uda.nvid>1
      ...
      switch dlt % setup DLT coefficients of desired
        case 'yes'
          ...
          uda.dltFileName=[fname1,pname1]; % store path and filename
        ...
        case 'no'
          uda.dlt=0; % DLT off
          uda.dltFileName=[]; % store null path and filename
          uda.dltcoef=[]; % empty dlt coefficients
    ...
    % turn the Load project button into the Save project button
    set(h(16),'string','Save project','ToolTipString',...
      'Save the complete DLTdv7 project as one file', ...
      'Callback',@dvSaveProject_v1)
    
    % Create the timeseries window
    h=tsInitWindow_v1(h,uda);
  ...
  case {2} % handle keypresses in the figure window - zoom & unzoom axes
    ...
    elseif cc=='f' || cc=='b' || cc=='F' || cc=='B' || cc=='<' || cc=='>' && axh~=0
      ...
      if cc=='f' && fr+1 <= smax
        if (get(h(35),'Value')==3 || get(h(35),'Value')==5) ...
          ...
          uda=DLTautotrack3fun(uda,h,keyadvance,axn,cp,fr,sp);
        end
    ...
    elseif cc=='R' % recompute 3D locations
      ...
      if isempty(filename)==false
        ...
        uda.dltFileName=[pathname,filename];
      end
    ...
    elseif cc=='U' % undo prior whole-point delete/swap/split/join
      ...
      if strcmp(button,'Yes')
        ...
        %DLTdv7(1,uda); % call self to update video frames
        fullRedraw(uda);
  ...
  case {3} % handle button clicks in axes
    ...
    if strcmp(seltype,'normal')==true && autoT>1
      ...
      elseif autoT>2 && autoT<5 % autoT = 3 (semi) or autoT = 4 (full)
        ...
        DLTautotrack3fun(uda,h,keyadvance,axn,cp,fr,sp);
      else
        fullRedraw(uda); % mostly for multi-track left or right click
      end
  ...
  case {5} % save data
    ...
    if uda.dlt
      ...
      if strcmp(CIs,'yes')==1
        ...
        if strcmp(minErrs,'yes')==1
          minErr=1.5; % adjust extra gain here
        else
          minErr=0;
        end
        ...
        [CI,tol,weights]=dlt_confidenceIntervalsSparse(uda.dltcoef,tdata,...
          minErr,uda);
        
        % spline filtered data
        if exist('spaps','file')~=2
          disp(['DLTdv7: The spline toolbox is not available so'...
          ...
          %  3*size(uda.dltpts,3));
          xyzData=uda.dltpts;
          xyzData=dlt_splineInterpSparse(xyzData,'linear');
          weights(isnan(weights))=0;
          fData=dlt_splineFilterSparse(xyzData,tol,weights,0);
          %fData(weights==0)=NaN;
        end
      end
    end
    ...
    if strcmpi(sType,'flat')
      ...
      if strcmp(overwrite,'Yes')==1
        ...
        if exist('fData','var')==1
          ...
          for i=1:size(fData,1)
            for j=1:numel(fData(i,:))-1
              fprintf(f1,'%.6f,',sp2full(fData(i,j)));
            end
            fprintf(f1,'%.6f\n',sp2full(fData(i,j+1)));
          end
          fclose(f1);
        ...
        uda.recentlysaved=1;
        setappdata(h(1),'Userdata',uda); % pass back complete user data
        msgString=['Data exported as ',[pname,filesep,pfix],'*.csv'];
        msgbox(msgString);
        disp(msgString);
      end
      ...
      if strcmpi(overwrite,'yes')
        ...
        uda.recentlysaved=1;
        setappdata(h(1),'Userdata',uda); % pass back complete user data
        msgString=['Data exported as ',[pname,filesep,pfix],'*.tsv'];
        msgbox(msgString);
        disp(msgString);
  ...
  case {6} % load previously saved points
    ...
    disp(['Loaded points from ',pfix,sfix])
  ...
  case {13} % Window close via non-Matlab method
    ...
    if uda.recentlysaved==0
      ...
      if strcmp(savefirst,'yes')
        %DLTdv7(5,uda); % call self to save data
        dvSaveProject_v1(uda.handles(29)); % the parent of h(29) is h(1) so this is compatible with dvSaveProject_v1
...
function [uda,fr]=DLTautotrack3fun(uda,h,keyadvance,axn,cp,fr,sp)

% DLTautotrack3.m function
%
% This function handles the auto-tracking (both semi and auto mode)
% functions for the DLTdv7.m
%
% Inputs:
...
% Version 3, Ty Hedrick, 2018-08-26 [re-implement multi-track]
...
% setup axn (axis number) matrix
if atMode==5 && h(63).Value==1 % multi-mode & all update videos
  AAxn=1:uda.nvid;
...
% grab the image caches
oData.cdataCache=getappdata(h(1),'cdataCache');
oData.cdataCacheIdx=getappdata(h(1),'cdataCacheIdx');
...
for A=1:numel(AAxn)
  ...
  if atMode==5 % multi-track mode
    spM(1,axn)=0; % make sure spM gets some value
    spInc=1; % selected point incrementer (loop counter)
    for i=1:size(uda.xypts,2)/(uda.nvid*2) % for each point
      % add to set if defined in this frame but not the next
      if uda.xypts(fr,axn*2-1+(i-1)*2*uda.nvid)~=0 && ...
          uda.xypts(fr+1,axn*2-1+(i-1)*2*uda.nvid)==0
        spM(spInc,A)=i;
        x=round(uda.xypts(fr,axn*2-1+(i-1)*2*uda.nvid));
        y=round(uda.xypts(fr,axn*2+(i-1)*2*uda.nvid));
...
while fr < frmax && trackFailed==false && get(h(35),'Value')~=1
  ...
  for A=1:numel(AAxn)
    ...
    oData.movs{axn}=mediaRead(fname,round(fr*uda.movFrameRateRatios(axn)+offset),get(h(12),'Value'),get(h(350+A),'Value'));
    imdat=oData.movs{axn}.cdata(:,:,:);
    oData.cdataCache{axn}=pushBuffer(oData.cdataCache{axn},imdat);
    oData.cdataCacheIdx{axn}=pushBuffer(oData.cdataCacheIdx{axn},round(fr*uda.movFrameRateRatios(axn)+offset));
    ...
    for S=1:numel(find(spM(:,A)>0))
      ...
      [x,y]=AutoTrackPredictor(otherpts,fr-1,get(h(57),'Value'));
  ...
  for sp=dltSet'
    ...
    if sum(uda.xypts(fr,(1:uda.nvid*2)+(sp-1)*2*uda.nvid)~=0)>=4 && ~isempty(uda.dltcoef) % 2+ xypts & DLT coefficients
      udist=sp2full(uda.xypts(fr,(1:2*uda.nvid)+(sp-1)*2*uda.nvid));
      for i=1:numel(udist)./2
        if isempty(uda.camud{i})==false
          udist(1,i*2-1:i*2)=applyTform(uda.camud{i},udist(1,i*2-1:i*2));
...
if uda.dlt && idx<=3
  ...
  plot(h(601),frPts,xyzPts,'HitTest','on','ButtonDownFcn',@tsSelectTrackByClick);
  hold(h(601),'on')
...
if doXY
  ...
  frPts=repmat([pseq';NaN],numel(ptarray),1);
  plot(h(601),frPts,xyPts,'HitTest','on','ButtonDownFcn',@tsSelectTrackByClick);
  hold(h(601),'on')
...
if isempty(filename)==false
  ...
  disp('Recomputing 3D coordinates cancelled.')
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% dvSaveProject_v1
function [] = dvSaveProject_v1(varargin)

% function [] = dvSaveProject_v1(hObject)
%
% Function to save the whole DLTdv7 project as a mat-file
...
% write some info for the save version
udExport.saveTime = datestr(now,'yyyy-mm-dd HH:MM:SS.FFF AM');
udExport.DLTdv_version = 7.0;

% copy out the essentials from the actual arrays
udExport.data.nvid = uda.nvid;
udExport.data.dltpts = uda.dltpts;
udExport.data.dltres = uda.dltres;
udExport.data.offset = uda.offset;
udExport.data.xypts = uda.xypts;
udExport.data.numpts = uda.numpts;
udExport.data.sp = uda.sp;
udExport.data.reloadVid = true; % set this for better re-initialization
udExport.data.gapJump = uda.gapJump;
udExport.data.movsizes = uda.movsizes;
udExport.data.drawVid = uda.drawVid;
udExport.data.camd = uda.camd;
udExport.data.camud = uda.camud;
udExport.data.dltcoef = uda.dltcoef;
udExport.data.dltFileName=uda.dltFileName;
udExport.data.dlt = uda.dlt;
udExport.data.lastvnum = uda.lastvnum;
udExport.data.lastaxh = uda.lastaxh;
udExport.data.movFrameRates = uda.movFrameRates;
udExport.data.movFrameRateRatios = uda.movFrameRateRatios;

% save UI settings
% grab enable settings for first 100 handles
for i=1:100
  try
    udExport.ui2.enable{i}=get(uda.handles(i),'enable');
  catch
    % nothing
  end
  try
    udExport.ui2.strings{i}=get(uda.handles(i),'string');
  catch
    % nothing
  end
  try
    udExport.ui2.callbacks{i}=get(uda.handles(i),'callback');
  catch
    % nothing
  end
end

% get properties for specific UI elements
udExport.ui.gamma.props.value = get(uda.handles(13),'value'); % value
udExport.ui.gamma.handle = 13; % handle ID
udExport.ui.color.props.value = get(uda.handles(12),'value');
udExport.ui.color.handle = 12;
udExport.ui.background.props.value = get(uda.handles(15),'value');
udExport.ui.background.handle = 15;
udExport.ui.frameText.props.value = get(uda.handles(8),'value');
udExport.ui.frameText.handle = 8;
udExport.ui.frameSlider.props.value = get(uda.handles(5),'value');
udExport.ui.frameSlider.props.Max = get(uda.handles(5),'Max');
udExport.ui.frameSlider.props.Min = get(uda.handles(5),'Min');
udExport.ui.frameSlider.props.SliderStep = get(uda.handles(5),'SliderStep');
udExport.ui.frameSlider.handle = 5;
udExport.ui.autoMode.props.value = get(uda.handles(57),'value');
udExport.ui.autoMode.handle = 57;
udExport.ui.autoPredictor.props.value = get(uda.handles(35),'value');
udExport.ui.autoPredictor.handle = 35;
udExport.ui.autoThreshold.props.value = get(uda.handles(39),'value');
udExport.ui.autoThreshold.handle = 39;
udExport.ui.autoSize.props.value = get(uda.handles(38),'value');
udExport.ui.autoSize.handle = 38;
udExport.ui.currentPoint.props.value = get(uda.handles(32),'value');
udExport.ui.currentPoint.handle = 32;

udExport.ui.dltThreshold.props.value = get(uda.handles(61),'value');
udExport.ui.dltThreshold.handle = 61;
udExport.ui.dltFeedback.props.value = get(uda.handles(53),'value');
udExport.ui.dltFeedback.handle = 53;

udExport.ui.vidUpdate.props.value = get(uda.handles(63),'value');
udExport.ui.vidUpdate.handle = 63;

udExport.ui.show2Dtracks.props.value = get(uda.handles(65),'value');
udExport.ui.show2Dtracks.handle = 65;

udExport.ui.findCentroid.props.value = get(uda.handles(55),'value');
udExport.ui.findCentroid.handle = 55;
udExport.ui.findCentroidColor.props.value = get(uda.handles(59),'value');
udExport.ui.findCentroidColor.handle = 59;

udExport.tsui.tsWidth.props.string = get(uda.handles(602),'string');
udExport.tsui.tsWidth.handle = 602;

udExport.tsui.tsMenu.props.string = get(uda.handles(603),'string');
udExport.tsui.tsMenu.props.value = get(uda.handles(603),'value');
udExport.tsui.tsMenu.handle = 603;

% videos & video-specific info
for i=1:uda.nvid
  % video filename
  vname=getappdata(uda.handles(300+i),'fname');
  if isstr(vname) % simple string
    udExport.video.files{i}=vname;
  else
    udExport.video.files{i}=[vname.Path,filesep,vname.Name];
  end
  
  % Video window UI elements
  udExport.video.offset{i}=get(uda.handles(325+i),'string');
  udExport.video.mirror{i}=get(uda.handles(350+i),'value');
  udExport.video.undistortString{i}=get(uda.handles(425+i),'string');
  udExport.video.frameRateString{i}=get(uda.handles(450+i),'string');
  
end

% secondary data stores
udExport.currframe = getappdata(h(1),'currframe');

% get file save name and location
[filename, pathname] = uiputfile('*.mat', 'Save DLTdv7 project as?');
if filename==0
  disp('Project save canceled')
  return
end

% setup filename with appended _dvProject suffix, but don't double-up the
% suffix
filename2 = filename(1:end-4);
s=strfind(filename2,'_dvProject');
if isempty(s)==false
  filename2=filename2(1:s-1);
end
filename2 = [filename2,'_dvProject.mat'];
udExport.savePath=pathname;
udExport.saveName=filename(1:end-4);
save([pathname,filename2],'udExport','-v7.3');
disp(['Project saved as ',pathname,filename2]);

% update saved status
uda.recentlysaved=true;
setappdata(h(1),'Userdata',uda);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% dvLoadProject_v1
function [] = dvLoadProject_v1(varargin)

% function [] = dvLoadProject_v1(hObject)
%
% Function to load the whole DLTdv7 project from a mat-file
...
% disable project loading in the UI
set(uda.handles(6),'enable','off');
set(uda.handles(16),'enable','off');

% browse for the project file
[filename,pathname] = uigetfile({'*_dvProject.mat','DLTdv7 project'},...
  'Pick a *_dvProject.mat file to load');
if filename==0
  disp('Load project canceled')
  % enable project loading in the UI
  set(uda.handles(6),'enable','on');
  set(uda.handles(16),'enable','on');
  return
end

% create the waitbar
wh = waitbar(0,['Loading ',pathname,filesep,filename]);
wh.Children.Title.Interpreter='none';
pause(0.001); % allow drawing update

% load the file
load([pathname,filename],'udExport');
waitbar(1/7,wh);

% restore basic data values
fn=fieldnames(udExport.data);
for i=1:numel(fn)
  eval(['uda.',fn{i},'=udExport.data.',fn{i},';']);
end

% restore UI enable on/off
for i=1:numel(udExport.ui2.enable)
  if isempty(udExport.ui2.enable{i})==false
    set(uda.handles(i),'enable',udExport.ui2.enable{i});
  end
end

% restore UI enable on/off
for i=1:numel(udExport.ui2.strings)
  if isempty(udExport.ui2.strings{i})==false
    set(uda.handles(i),'string',udExport.ui2.strings{i});
  end
end

% restore UI callbacks
for i=1:numel(udExport.ui2.callbacks)
  if isempty(udExport.ui2.callbacks{i})==false
    set(uda.handles(i),'callback',udExport.ui2.callbacks{i});
  end
end

% restore UI values
fn=fieldnames(udExport.ui);
for i=1:numel(fn)
  eval(sprintf('fn2=fieldnames(udExport.ui.%s.props);',fn{i}));
  for j=1:numel(fn2)
    eval(sprintf('set(uda.handles(udExport.ui.%s.handle),%s%s%s,udExport.ui.%s.props.%s);',fn{i},char(39),fn2{j},char(39),fn{i},fn2{j}));
  end
end

% restore 2ndary data stores
setappdata(uda.handles(1),'currframe',udExport.currframe);
...
setappdata(uda.handles(1),'cdataCache',cdataCache);
setappdata(uda.handles(1),'cdataCacheIdx',cdataCacheIdx);
waitbar(2/7,wh);
...
for i=1:uda.nvid
  set(uda.handles(200+i),'visible','off');
end
waitbar(3/7,wh);
...
for i=1:uda.nvid
  ...
  setappdata(uda.handles(300+i),'Userdata',uda);
  setappdata(uda.handles(200+i),'Userdata',uda);
  set(uda.handles(325+i),'string',udExport.video.offset{i});
  set(uda.handles(350+i),'value',udExport.video.mirror{i});
  set(uda.handles(425+i),'string',udExport.video.undistortString{i});
  set(uda.handles(450+i),'string',udExport.video.frameRateString{i});
  
  set(gca,'XTickLabel','');
  set(gca,'YTickLabel','');
  setappdata(uda.handles(i+300),'Userdata',uda);
  set(get(uda.handles(i+300),'Children'),'ButtonDownFcn','DLTdv7(3)');
  %setappdata(get(uda.handles(i+300),'Children'),'Userdata',uda);
  
end
waitbar(4/7,wh);

% create & restore timeseries window
uda.handles=tsInitWindow_v1(uda.handles,uda);
fn=fieldnames(udExport.tsui);
for i=1:numel(fn)
  eval(sprintf('fn2=fieldnames(udExport.tsui.%s.props);',fn{i}));
  for j=1:numel(fn2)
    eval(sprintf('set(uda.handles(udExport.tsui.%s.handle),%s%s%s,udExport.tsui.%s.props.%s);',fn{i},char(39),fn2{j},char(39),fn{i},fn2{j}));
  end
end
waitbar(5/7,wh);

% restore video images
for i=1:uda.nvid
  set(uda.handles(i+300),'Visible','on');
  [mov,fname]=mediaRead(getappdata(uda.handles(300+i),'fname'),udExport.currframe(i),false,false); % grab the current frame
  setappdata(uda.handles(i+300),'fname',fname); % set the name (or mmreader obj)
  redlakeplot(mov.cdata(:,:),uda.handles(i+300));
  colormap(uda.handles(i+300),gray(256));
end

% make invisible for now
for i=1:uda.nvid
  set(uda.handles(200+i),'visible','on');
end
waitbar(6/7,wh);
...
warning off MATLAB:HandleGraphics:ObsoletedProperty:JavaFrame
for i=1:uda.nvid
  jFig = get(uda.handles(200+i), 'JavaFrame');
  jAxis = jFig.getAxisComponent;
  cc=jAxis.countComponents;
  for j=1:cc
    set(jAxis.getComponent(j-1),'FocusGainedCallback',{@figFocusFunc,uda.handles(200+i)});
  end
end
...
% update saved status
uda.recentlysaved=true;
setappdata(uda.handles(1),'Userdata',uda);

% close waitbar
close(wh)
disp(['Loaded ',pathname,filesep,filename])

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% dvMakeMovieWindows
function [uda] = dvMakeMovieWindows(uda)

% function [uda] = dvMakeMovieWindows(uda)
%
% Function for creating the video windows and handles
...
for i=1:uda.nvid
  h(200+i) = figure('units','pixels', 'color',[0.83 0.82 0.78],...
    'doublebuffer','on','KeyPressFcn','DLTdv7(2)','menubar','none',...
    'Name',sprintf('Movie %.0f',i),'numbertitle','off',...
    'handlevisibility','callback','tag',sprintf('movieFig%.0f',i),...
    'Userdata',uda,'Visible','on','deletefcn','DLTdv7(13)',...
    'pointer','cross','interruptible','on','position',...
    [361+i*10 242-i*10 560 420],'WindowScrollWheelFcn',@dvScrollWheel);
  ...
  h(300+i) = axes('Position',[.01 .10 .99 .89],'XTickLabel','', ...
    'YTickLabel','','ButtonDownFcn','DLTdv7(3)', ...
  ...
  uicontrol('Parent',h(200+i),... % frame offset label
    ...
    'Position',[12 1 20 2],'String','Frame offset:',...
    'Style','text','Tag','text4');
  
  h(325+i) = uicontrol('Parent',h(200+i),... % frame offset box
    'Units','characters','BackgroundColor',[1 1 1],'Position',...
    [34 1.5 7 2],'String','0','Style','edit','Tag','edit2', ...
    'Callback','DLTdv7(8)','enable','on');
  
  h(350+i) = uicontrol('Parent',h(200+i),... % horizontal mirror checkbox
    'Units','characters','BackgroundColor',[1 1 1],'Position',...
    [42 1.5 14 2],'String','H. mirror','Style','checkbox',...
    'Tag','edit2','Callback','DLTdv7(17)','enable','on','visible','on', ...
    'HorizontalAlignment','center');
  
  h(400+i) = uicontrol('Parent',h(200+i),... % undistortion button
    'Units','characters','BackgroundColor',[1 1 1],'Position',...
    [57 1.5 26 2],'String','Set Undistortion profile','style','pushbutton',...
    'Tag','UndistortionButton','Callback','DLTdv7(18)','enable','on',...
    'Visible','on');
  
  h(425+i) = uicontrol('Parent',h(200+i), ... % undistortion file name
    'Units','characters','BackgroundColor',[0.83 0.82 0.78],...
    'HorizontalAlignment','left',...
    'Position',[57 0 40 2],'String','Undistortion file: none',...
    'Style','text','Tag','text4','visible','on');
  
  uicontrol('Parent',h(200+i),... % frame rate label
    'Units','characters','BackgroundColor',[0.83 0.82 0.78],...
    'HorizontalAlignment','right',...
    'Position',[0 1 10 2],'String','Frame rate (Hz):',...
    'Style','text','Tag','frameRateText');
  
  h(450+i) = uicontrol('Parent',h(200+i),... % frame rate edit box
    'Units','characters','BackgroundColor',[1 1 1],'Position',...
    [11 1.5 7 2],'String','0','Style','edit','Tag','frameRateEdit', ...
    'Callback',@dvFrameRateUpdate,'enable','on');
...
% get master uda
h=get(gcbo,'parent');
ud=getappdata(h,'Userdata');
uda=getappdata(ud.handles(1),'Userdata');

% which movie am I
idx=find(h==uda.handles(201:225));

% what's the new frame rate?
frNew=str2num(get(gcbo,'string'));

% check input validity
if isempty(frNew) || frNew<=0
  set(gcbo,'string',num2str(uda.movFrameRates(idx)));
  disp('Video frame rates must be numeric and positive.')
  return
end

% update userdata
uda.movFrameRates(idx)=frNew;
uda.movFrameRateRatios=uda.movFrameRates./uda.movFrameRates(1);
setappdata(ud.handles(1),'Userdata',uda);
...
%  uda - userdata structure from DLTdv7
%
% Outputs:
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% dlt_confidenceIntervalsSparse

function [CI,tol,weights]=dlt_confidenceIntervalsSparse(coefs,camPts,minErr,uda)
...
if exist('oData','var')==true
  cdataCache=oData.cdataCache;
  cdataCacheIdx=oData.cdataCacheIdx;
  %disp('got image cache data from autotrack loop')
else
  cdataCache=getappdata(uda.handles(1),'cdataCache');
  cdataCacheIdx=getappdata(uda.handles(1),'cdataCacheIdx');
end
...
if strcmp(get(h(5),'enable'),'on')==1 % if initialized
  ...
  for i=vidlist
    ...
    if uda.drawVid(i)==true || get(h(63),'Value')==true
      ...
      % check the cache
      % disp(fr+offset)
      idx=find(cdataCacheIdx{i}==round(fr*uda.movFrameRateRatios(i)+offset));    
      if numel(idx)>0 & uda.reloadVid==false
        mov.cdata=cdataCache{i}{idx(1)};
        %disp('Got frame from cache')
      elseif round(fr*uda.movFrameRateRatios(i)+offset) <= 0 || round(fr*uda.movFrameRateRatios(i)+offset)>frmax
        % no image available
        mov.cdata=chex(1,round(xl(2)),round(yl(2)));
        cdataCache{i}=pushBuffer(cdataCache{i},mov.cdata);
        cdataCacheIdx{i}=pushBuffer(cdataCacheIdx{i},round(fr*uda.movFrameRateRatios(i)+offset));
        %cdataCache{i}=mov.cdata;  % cache the image
        
      elseif round(fr*uda.movFrameRateRatios(i)+offset) ~= currframe(i) || uda.reloadVid==true
        ...
        try
          if exist('oData','var')
            ...
            if isempty(mov)
              mov=mediaRead(fname,round(fr*uda.movFrameRateRatios(i)+offset),get(h(12),'Value'),...
                get(h(350+i),'value'));
            end
          else
            mov=mediaRead(fname,round(fr*uda.movFrameRateRatios(i)+offset),get(h(12),'Value'),...
      ...
      if bsubVal==true
        ...
        %idx=find(cdataCacheIdx{i}==round(fr*uda.movFrameRateRatios(i)+bsubFrame+offset));
        fOff=cdataCacheIdx{i}-round(fr*uda.movFrameRateRatios(i)+offset);
        idx=find(fOff<=-3 & fOff>=-6);
        if numel(idx)>0
          idx=flipud(sortrows([fOff(idx),idx]));
          idx=idx(1,2);
      ...
      currframe(i)=round(fr*uda.movFrameRateRatios(i)+offset);
      set((h(i+300)),'XTickLabel','');
      set((h(i+300)),'YTickLabel','');
      set(h(i+300),'ButtonDownFcn','DLTdv7(3)');
      setappdata(get(h(i+300),'Children'),'Userdata',ud);
...
%% tsInitWindow_v1
function [h] = tsInitWindow_v1(h,uda)
...
h(600) = figure('Units','pixels',... % timeseries figure
  'Color',[0.831 0.815 0.784]','Doublebuffer','on', ...
  'IntegerHandle','off','MenuBar','none',...
  'Name','DLTdv7 timeseries','NumberTitle','off',...
  'Position',[20 20 800 400],'Resize','on',...
  'HandleVisibility','callback','Tag','ts_figure',...
  'Userdata',[],'Visible','on','deletefcn','DLTdv7(13)',...
  'interruptible','on','KeyPressFcn',@tsKeyPress);
...
h(602) = uicontrol('Parent',h(600),...
  'Units','characters','BackgroundColor',[1 1 1],'Position',...
  [24 1.5 9 2],'String',num2str(size(uda.xypts,1)),'Style','edit',...
...
uicontrol('Parent',h(600),...
  'Units','characters','BackgroundColor',[0.83 0.82 0.78],...
  'HorizontalAlignment','right',...
  'Position',[37 1 8 2],'String','Display:',...
...
h(603) = uicontrol(...
  ...
  [46 1.5 11 2],'String',s,'Style','popupmenu',...
...
ud.handles=h;
for i=600:603
  setappdata(h(i),'Userdata',uda);
end

function [] = tsKeyPress(a,b)

ud=getappdata(a,'Userdata');
h=ud.handles;
uda=getappdata(h(1),'Userdata'); % full data bundle
cc=b.Character;
sp=get(h(32),'Value'); % selected point
...
elseif cc=='f' || cc=='b' || cc=='F' || cc=='B' || cc=='<' || cc=='>' && axh~=0
  ...
  elseif cc=='<' || cc=='>' % switch to start or end of this point in this camera
    ptval=get(h(32),'Value'); % selected point
    idx=find(uda.xypts(:,axn*2-1+(sp-1)*2*uda.nvid)~=0);
    %idx=find(isfinite(uda.xypts(:,vnum*2,ptval)));
    if numel(idx)>0
      if cc=='<'
        set(h(5),'Value',idx(1));
...
fullRedraw(uda)
% update the magnified point view
cp=sp2full(uda.xypts(fr,(uda.lastvnum*2-1:uda.lastvnum*2)+(sp-1)*2*uda.nvid)); % set current point to xy value
updateSmallPlot(uda.handles,uda.lastvnum,cp);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% selectTrackByClick
function [] = tsSelectTrackByClick(src,eventdata)
...
idx=get(uda.handles(603),'value');

% get frame number
frClick=round(eventdata.IntersectionPoint(1));
fr=round(get(uda.handles(5),'Value')); % current frame #

if uda.dlt && idx<=3
  % xyz data
  vals=sp2full(uda.dltpts(frClick,(idx:3:end)));
...
if doXY
  % xy data
  vals=sp2full(uda.xypts(frClick,(idx:uda.nvid*2:end)));
  d=abs(vals-eventdata.IntersectionPoint(2));
end
...
set(uda.handles(32),'Value',sp);
uda.sp=sp;

% get new xy location in the frame in question (for magnified plot)
pt=uda.xypts(fr,(uda.lastvnum*2-1:uda.lastvnum*2)+(sp-1)*2*uda.nvid);
...
% do a quick screen redraw
quickRedraw(uda,uda.handles,sp,fr);
